...
void LCD_FSMCConfig(void){
    ...
    gpio_af_set(GPIOE, GPIO_AF_12, GPIO_PIN_7|GPIO_PIN_8
		...
		// EXMC功能
    gpio_af_set(GPIOG, GPIO_AF_12, GPIO_PIN_0|GPIO_PIN_12);
...
//初始化
void NT35510_HY35_Initial_Code(void){
...
void LCD_Init(uint8_t dir)
{
    ...
    if(dir==crosswise)		//	//横向显示
    {	
        LCD_Scan_Dir(D2U_L2R);
        lcddev.dir=crosswise;
...
void LCD_ShowPic(unsigned int Xstart,unsigned int Xend,unsigned int Ystart,unsigned int Yend,const uint16_t  *pic,int length){
	//发送范围
    BlockWrite(Xstart,Xend,Ystart,Yend);
    LCD_WriteRAM_Prepare();       //开始写入GRAM命令
    for(int index=0;index<length;index++)
    {
        LCD_WriteRAM(pic[index]);
    }
}
