...
/******************************************************************************
      函数说明：在指定区域填充颜色
      入口数据：xsta,ysta   起始坐标(矩形左上角)
                xend,yend   终止坐标(矩形右下角)
								color       要填充的颜色
      返回值：  无
******************************************************************************/
...
{
	// 变量定义和初始化
	uint8_t i,j,m=0; // i、j、m为循环计数器，m用于计算每一行已经绘制了几个像素点
	uint16_t k; // k为汉字点阵数据的下标
	uint16_t HZnum; // 汉字数目
	uint16_t TypefaceNum; // 一个字符所占字节大小
	uint16_t x0=x; // 保存x坐标的初始值
	...
	for(k=0;k<HZnum;k++) 
	{
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				for(j=0;j<8;j++)
				{	
                    // 判断当前像素点应该绘制什么颜色，写入显示屏指定位置
					if(!mode) // 非叠加模式
					{
						if(tfont24[k].Msk[i]&(0x01<<j)) // 该像素点应该绘制为前景色
							Lcd_WriteData16(fc);
						else // 该像素点应该绘制为背景色
							Lcd_WriteData16(bc);

						m++;
						if(m%sizey==0) // 如果绘制了一整行像素点，则结束本行的绘制
						{
							m=0;
							break;
						}
					...
					else // 叠加模式
					{
						...
						x++; // x坐标自增
						if((x-x0)==sizey) // 如果绘制了一整行像素点，则结束本行的绘制
						{
							x=x0;
						y++;
						break;
